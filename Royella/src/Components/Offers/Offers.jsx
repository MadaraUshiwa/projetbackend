...
const Offers = () => {
  ...
  console.log(offer);
  const [randomoffre, setrandomoffre] = useState([]);
  ...
  useEffect(() => {
    ...
    if (offer && offer.length > 0) {
      while (randomoffre.length < 4) {
        let random = Math.floor(Math.random() * offer.length);
        if (!randomoffre.includes(offer[random])) {
          randomoffre.push(offer[random]);   
        }
      }
      setrandomoffre(randomoffre);
    }}, [offer]);

  return (
    <>
      {offer ?     <section className="bg-[#f8f6f3] dark:bg-lightBlack">
      <div className="Container py-20 lg:py-[120px] ">
        ...
        {randomoffre ? randomoffre.map((room, index) => (
              ...
              >
                <div className="relative">
                  <img
                    ...
                    className="h-[330px] w-[300px] object-cover"
                    alt=""
                  />
                </div>
