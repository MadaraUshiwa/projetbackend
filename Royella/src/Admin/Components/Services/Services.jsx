import './Services.css'
import { Link } from 'react-router-dom'
import axios from 'axios'
import { useEffect, useState } from 'react'

export default function Services() {

    const [services, setServices] = useState(null);

    useEffect(() => {
        axios.get("http://127.0.0.1:8000/api/services/")
            .then((response) => {
                if (response.data.services) {
                    setServices(response.data.services);
                }
            })
            .catch((error) => {
                console.log(error);
            });
    }, []);

    return (
        <div className='update-home h-full w-[100%]' data-aos="fade-up" data-aos-duration="1000" >
            <h1 className='titre pl-5 pt-3'><b>SERVICES</b></h1>
            <div className='update-home-content py-5 px-5 h-[90%] w-[100%]'>
                <div className='facilities-container h-[100%]'>
                    <div className='backoffice-header'>
                        <h3>Services</h3>
                    </div>
                    <div className="table-services-container h-[80%]">
                        <table className='h-full'>
                            <thead>
                                <tr scope='col' className='first-col'>ID</tr>
                                <tr scope='col'>Titre</tr>
                                <tr scope='col'>Theme</tr>
                                <tr scope='col' className='order-col'>Ordre</tr>
                                <tr scope='col'>Description</tr>
                                <tr scope='col'>Image</tr>
                                <tr scope='col' className='services-last-col'></tr>
                            </thead>
                            <tbody>
                                {services ? services.map((service, index) => (
                                    <tr key={index}>
                                        <th scope='row' className='first-col'>{service.id}</th>
                                        <td>{service.titre}</td>
                                        <td>{service.theme}</td>
                                        <td className='order-col'>{service.ordre}</td>
                                        <td className='flex items-start'>{service.description}</td>
                                        <td><img src={`http://127.0.0.1:8000${service.image}`}></img></td>
                                        <td className='services-last-col'>
                                            <Link to={`/admin/services/update/${service.id}`}>
                                            <i className="gg-edit-markup"></i>
                                            </Link>
                                        </td>
                                    </tr>
                                )) : null}
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    )
}